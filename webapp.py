...
DAILY_REPORT_PATH = f"db/market_{MARKET_ID}_daily_report.csv"
...
RELEVANT_METRICS = [
    "lastTradePrice", "bestBid", "bestAsk", "volume", "volume24hr", "liquidity", "umaBond", "umaReward", "competitive", "rewardsMinSize", "rewardsMaxSpread", "spread", "oneDayPriceChange"
]

# -------- Create Dash app --------
app = dash.Dash(__name__)
app.title = f"Polymarket Dashboard - # {MARKET_ID} - {df['question'].iloc[0]}"

# -------- Layout --------
app.layout = html.Div(style = {"fontFamily": "Arial", "padding": "2rem", "backgroundColor": "#f9f9f9"}, children = [
    html.H1(f"Polymarket Market - #{MARKET_ID} - {df['question'].iloc[0]}", style = {"textAlign": "center", "color": "#333"}),
    ...
    html.Div([
        ...
        dcc.Dropdown(
            id = "metric-selector",
            options = [{"label": col, "value": col} for col in RELEVANT_METRICS if col in df.columns],
            value = ["lastTradePrice", "bestBid", "bestAsk"],
            multi = True,
...
def update_layout(n, time_range, selected_metrics):
    ...
    metrics_block = html.Div([
        ...
        html.P(f"{metric}: {latest[metric]}" if metric in latest else f"{metric}: N/A")
    ...
    health_info = [
        ("Market Question", latest.get("question", "N/A")),
        ("Condition ID", latest.get("conditionId", "N/A")),
        ("Start Date", latest.get("startDate", "N/A")),
        ("End Date", latest.get("endDate", "N/A")),
        ("Resolution Source", latest.get("resolutionSource", "N/A")),
        ("Restricted", str(latest.get("restricted", "N/A"))),
        ("Archived", str(latest.get("archived", "N/A"))),
        ("Accepting Orders", str(latest.get("acceptingOrders", "N/A"))),
        ("Closed", str(latest.get("closed", "N/A"))),
        ("Active", str(latest.get("active", "N/A")))
    ]
